...
const compareFields = (data, serviceName, _schema) => {
    const requiredFields = _schema.data.filter((f) => f.properties?.required === true).map((f) => f?.field);
    const objectIdFields = _schema.data.filter((f) => f.properties?.type === 'ObjectId').map((f) => f?.field);
