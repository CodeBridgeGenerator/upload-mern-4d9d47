...
import studentDetailsFakerFactory from "./studentDetailsFakerFactory";
...
const StudentDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = studentDetailsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("studentDetails").create(elem));
...
export default connect(mapState, mapDispatch)(StudentDetailsFakerDialogComponent);
