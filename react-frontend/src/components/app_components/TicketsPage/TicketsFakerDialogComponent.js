...
import ticketsFakerFactory from "./ticketsFakerFactory";
...
const TicketsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = ticketsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("tickets").create(elem));
...
export default connect(mapState, mapDispatch)(TicketsFakerDialogComponent);
