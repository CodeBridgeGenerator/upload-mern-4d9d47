    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const employeesResponse = await client.service('employees').find();
                ...
                // Count number of employees per company using the company ID from employee data
                const employeeCounts = employees.reduce((acc, employee) => {
                    const companyId = employee?.company?.$oid || employee?.company;
                    if (companyId) {
                ...
                companies.forEach((company) => {
                    ...
                    if (employeeCounts[companyId]) {
                        labels.push(company.name); // Add company name as label
                        data.push(employeeCounts[companyId]); // Add employee count
                    }
                });
