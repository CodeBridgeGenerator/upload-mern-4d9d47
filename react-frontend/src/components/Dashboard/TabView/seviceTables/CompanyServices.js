        ...
        const fetchData = async () => {
            ...
            try {
                if (activeIndex === 0) {
                    ...
                    // Fetch branches data
                    const res = await client.service('branches').find({ query: { $limit: 5 } });
                    setBranchData(res.data);
                } else if (activeIndex === 2) {
                    // Fetch departments data
                    const res = await client.service('departments').find({ query: { $limit: 5 } });
                ...
                <div className="companies-table">
                    {loading ? (
                        ...
                        <DataTable value={companyData} rows={5}>
                            ...
                            <Column field="newCompanyNumber" header="New Company Number" sortable />
                            <Column field="DateIncorporated" header="Date Incorporated" sortable />
                        </DataTable>
                    )}
                </div>
            </TabPanel>
